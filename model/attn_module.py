...
class Model(Base):
    ...
    def __init__(self, observation_shape, num_actions, room_height, room_width, vocab, demb, drnn, drep, pretrained_emb=False, disable_wiki=False):
        ...
        self.film1 = FILM(2*drnn, 16, nn.Conv2d(demb+2, 16, kernel_size=(3, 3), padding=1))
    ...
    def fuse(self, inputs, cell, inv, wiki, task):
        ...
        dtext = text.size(1)
        text = text.unsqueeze(2).unsqueeze(2).expand(T*B, dtext, W, H)  # (T*B, dtext, W, H) = (T*B, 6*drnn, W, H)
        
        tb = torch.flatten(cell, 0, 1)  # (T*B, H, W, demb)

        conv_in = tb.transpose(1, 3)  # (T*B, demb, W, H)
        conv_sim = (conv_in * text).sum(1, keepdim=True)  # (T*B, 1, W, H)
        attn_score = self.soft2d(conv_sim)  # (T*B, 1, W, H)
        conv_in = conv_in * attn_score  # (T*B, demb, W, H)
        pos = inputs['rel_pos'].float().view(T*B, H, W, -1).transpose(1, 3)  # (T*B, 2, W, H)
        ...
        c5 = self.film5(c4, pos)
        conv_out = c5.mean(3)[0].mean(2)[0]  # pool over spatial dimensions
